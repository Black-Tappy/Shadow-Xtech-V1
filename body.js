...
var session = conf.session.replace(/SHADOW-XTECH;;;=>/g,"");
...
authentification();
    async function main() {
        ...
        const sockOptions = {
            ...
            browser: ['Sʜᴀᴅᴏᴡ Xᴛᴇᴄʜ', "safari", "1.0.0"],
...
setInterval(async () => {
    if (conf.AUTO_BIO === "yes") {
        ...
        const bioText = `✨🌇 ꜱʜᴀᴅᴏᴡ xᴛᴇᴄʜ ɪs ✨ ᴀᴄᴛɪᴠᴇ ✨ 🟢 | ⏳ \n${currentDateTime} 📸 | 💬🌆`; // Format the bio text
...
let auto_reply_message = `Hello 👋, I'm ${conf.BOT} on board 🎲. My owner ${conf.OWNER_NAME} currently unavailable 🚫. Please leave a message 📨, and we will get back to you as soon as possible ⚪. Thanks To ${conf.DEV}`;
...
const audioMap = {
    ...
    "holla": "media/hey.mp3",
    "hello": "media/hey.mp3",
    "arise": "media/morning.mp3",
    "morning": "media/morning.mp3",
    "goodmorning": "media/morning.mp3",
    "wake up": "media/morning.mp3",
    "night": "media/goodnight.mp3",
    "goodnight": "media/goodnight.mp3",
    "sleep tight": "media/goodnight.mp3",
    "man": "media/hey.mp3",
    "awoh": "media/hey.mp3",
    "fty": "media/response.mp3",
    "fty mkuu": "media/response.mp3",
    "fiti": "media/response.mp3",
    "wazii": "media/response.mp3",
    "bot": "media/shadow-xtech.mp3",
    "bright": "media/shadow-xtech.mp3",
    "xbot": "media/shadow.mp3",
    "Black-Tappy": "media/shadow-xtech.mp3",
    "mdx": "media/shadow.mp3",
    "xmd": "media/shadow.mp3",
    "whatsapp bot": "media/shadow.mp3",
    "evening": "media/afternoon.mp3",
    "goodevening": "media/afternoon.mp3",
    "beiby": "media/love.mp3",
    "babe": "media/love.mp3",
    "love": "media/feelings.mp3",
    "sweetheart": "media/goodafternoon.wav",
    "nimekumiss": "media/hey.mp3",
    "byee": "media/pickuplines.mp3",
    "i love you too": "media/pickuplines.mp3",
    "my love": "media/feelings.mp3",
    "i love you": "pickuplines.mp3",
    "umbwa": "media/insult.mp3",
    "mjinga": "media/insult.mp3"
    "wtf": "media/insult.mp3",
    "fuck you": "media/insult.mp3",
    "bro": "media/hey.mp3",
    "ndugu": "media/hey.mp3",
    "morio": "audios/hey.mp3",
    "mzee": "media/hey.mp3",
    "kijana": "media/hey.mp3",
    "mkuu": "media/hey.mp3",
     "ozah": "media/hey.mp3",
     "ozaah": "media/hey.mp3",
    "oyaah": "media/hey.mp3",
    ...
    zk.ev.on("messages.upsert", async (m) => {
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            /* const bright='254759000340';
             ...
             const blacktappy='254756360306'*/
            /*  var superUser=[servBot,bright,black,blacktappy].map((s)=>s.replace(/[^0-9]/g)+"@s.whatsapp.net").includes(auteurMessage);
              var dev =[bright,black,blacktappy].map((t)=>t.replace(/[^0-9]/g)+"@s.whatsapp.net").includes(auteurMessage);*/
            ...
            const superUserNumbers = [servBot, blacktappy, black, bright, conf.NUMERO_OWNER].map((s) => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
            const allAllowedNumbers = superUserNumbers.concat(sudo);
            const superUser = allAllowedNumbers.includes(auteurMessage);
            
            var dev = [blacktappy,black,bright].map((t) => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
            function repondre(mes) { zk.sendMessage(origineMessage, { text: mes }, { quoted: ms }); }
            console.log("\t[][]...{shadow-Md}...[][]");
            console.log("=========== New message ===========");
            if (verifGroupe) {
                console.log("message from : " + nomGroupe);
            }
            console.log("message sent from : " + "[" + nomAuteurMessage + " : " + auteurMessage.split("@s.whatsapp.net")[0] + " ]");
...
var verifBrightAdmin = verifGroupe ? admins.includes(idBot) : false;
...
if (texte && texte.startsWith('>')) {
  ...
  if (!superUser) {
    const menuText = `This command is only for the owner or bot owner to execute 🚫`;
 ...
 //---------------------------------------rang-count--------------------------------
  ...
  const { ajouterOuMettreAJourUserData } = require("./database/level"); 
  try {
    ...
    console.error(e);
              ...
              try {
                ...
                if (ms.message[mtype].contextInfo.mentionedJid && (ms.message[mtype].contextInfo.mentionedJid.includes(idBot) ||  ms.message[mtype].contextInfo.mentionedJid.includes(conf.NUMERO_OWNER + '@s.whatsapp.net'))    /*texte.includes(idBot.split('@')[0]) || texte.includes(conf.NUMERO_OWNER)*/) {
                    ...
                    let mbd = require('./database/mention') ;
                                   ...
                                   // txt += `message supprimé \n @${auteurMessage.split("@")[0]} rétiré du groupe.`;
                                    ...
                                    await fs.unlink("st1.webp"); } 
                                       ...
                                       await fs.unlink("st1.webp");
                                        ...
                                        const {getWarnCountByJID ,ajouterUtilisateurAvecWarnCount} = require('./database/warn') ;
...
const { recupevents } = require('./database/welcome'); 
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        ppgroup = 'https://files.catbox.moe/3hrxbh.jpg';
    }
...
`;
            ...
            let msg = `📝 A member 👤has just left group ⚪. Lets pray for the poor🥺.Anyway Goodbye Hustler🌇.;\n`;
...
/******** fin d'evenement groupe update *************************/
    ...
    async  function activateCrons() {
        ...
        const { getCron } = require('./database/cron');
            ...
            for (let i = 0; i < crons.length; i++) {
              ...
              if (crons[i].mute_at != null) {
                ...
                cron.schedule(`${set[1]} ${set[0]} * * *`, async () => {
                  ...
                  zk.sendMessage(crons[i].group_id, { image : { url : './media/chrono.webp'} , caption: "Hello 👋, it's time to close🔒 the group 👥; members." });
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ Shadow-Xtech connecting to your account...");
            }
            else if (connection === 'open') {
                console.log("✅ Shadow-Xtech connected successfully to WhatsApp! ☺️");
                console.log("--");
                await (0, baileys_1.delay)(200);
                console.log("------");
                await (0, baileys_1.delay)(300);
                console.log("------------------/-----");
                console.log("Shadow-Xtech is Online 🟢\n\n");
                //chargement des commandes
                console.log("♻️ Loading Shadow-Xtech Plugins...\n");
                fs.readdirSync(__dirname + "/plugins").forEach((fichier) => {
                    if (path.extname(fichier).toLowerCase() == (".js")) {
                        try {
                            require(__dirname + "/plugins/" + fichier);
                            console.log(fichier + "🔒Shadow-Xtech Plugins Installed Successfully✔️");
                        }
                        catch (e) {
                ...
                console.log("Shadow-Xtech Plugins Installation Completed ✅");

                await activateCrons();
                
                if((conf.DP).toLowerCase() === 'yes') {     

                let cmsg =`*Hello there User! 👋🏻* 

> Simple, Straightforward, But Loaded With Features 🎊. Meet Shadow-Xtech WhatsApp Bot.

*Thanks for using Shadow-Xtech 🚩* 
...
https://github.com/Black-Tappy/Shadow-Xtech-V1
