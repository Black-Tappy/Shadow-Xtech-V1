    ...
    return repondre("💬 Please specify the book, chapter, and verse you want to read. Example: bible Mathew 3:16", {
      contextInfo: {
        externalAdReply: {
          ...
          thumbnailUrl: "https://files.catbox.moe/qmmtzg.jpg", // Replace with a suitable thumbnail URL
          sourceUrl: conf.GURL,
          mediaType: 1,
          showAdAttribution: true,
        },
...
zokou({
  ...
  if (!arg || arg.length === 0) {
    const replyText = "Example Usage: .code 2547590000xxx.";
    return repondre(replyText);
  }

  try {
    // Notify user that pairing is in progress
    const replyText = "*Sʜᴀᴅᴏᴡ-Xᴛᴇᴄʜ is getting your pair code 🧬✅...*";
    await repondre(replyText);

    // Prepare the API request
    const encodedNumber = encodeURIComponent(arg.join(" "));
    const apiUrl = `https://shadow-xtech.onrender.com?number=${encodedNumber}`;
...
*Sʜᴀᴅᴏᴡ-Xᴛᴇᴄʜ Element Information:*
👑 *Name:* ${data.name}
📡 *Symbol:* ${data.symbol}
...
Regards ${conf.BOT} `;
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "Sʜᴀᴅᴏᴡ-Xᴛᴇᴄʜ Eʟᴇᴍᴇɴᴛ ꀤɴꜰᴏʀᴍᴀᴛɪᴏɴ,
...
zokou({
  ...
  if (!githubUsername) {
    return repondre("💬 Give me a valid GitHub username like: github Black-Tappy");
  }
...
`;
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "Sʜᴀᴅᴏᴡ-Xᴛᴇᴄʜ Gɪᴛʜᴜʙ Uꜱᴇʀ ꀤɴꜰᴏ",
...
> Powered by Sʜᴀᴅᴏᴡ-Xᴛᴇᴄʜ 
    ...
    console.error(err);
    repondre(`🚫 Error 404. I did not find anything!`);
  }
});
