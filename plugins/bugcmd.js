...
// bug database
const { bugtext1 } = require("../framework/bright/bugtext1");
const { bugtext2 } = require("../framework/bright/bugtext2");
const { bugtext3 } = require("../framework/bright/bugtext3");
const { bugtext4 } = require("../framework/bright/bugtext4");
const { bugtext5 } = require("../framework/bright/bugtext5");
const { bugtext6 } = require("../framework/bright/bugtext6");
const { bugpdf } = require("../framework/bright/bugpdf.js");
...
async function relaybug(dest, zk, ms, repondre, amount, victims, bug) {
    ...
    repondre(`🔒 Successfully sent ♻️ ${amount} bugs to ${victims.join(", ")}.`);
}
...
zokou(
    ...
    async (dest, zk, commandOptions) => {
        ...
        if (arg.length === 1) {
            victims.push(arg[0]);
            await repondre(`♻️ sending ${amount} bugs to ${victims[0]}`);
            try {
            ...
            if (isNaN(amount)) {
                ...
                if (victims.length > 0) {
                    await repondre(
                        `♻️ sending ${amount} bugs to ${victims.join(", ")}`
                    );
                    try {
                        await sendbug(
                            dest,
...
zokou(
    ...
    async (dest, zk, commandOptions) => {
        ...
        await repondre(
            `*🔒 Successfully sent ♻️ as many bugs as ${amount} 💬 Please pause for 3 minutes*`
        );
        await react(dest, zk, ms, "✅");
    }
);
