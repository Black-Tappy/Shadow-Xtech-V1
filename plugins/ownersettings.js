...
zokou({
  ...
  if (!superUser) {
    return repondre("🚫 thís cσmmαnd ís rєstríctєd tσ thє вσt σwnєr. σr Sʜᴀᴅᴏᴡ-Xᴛᴇᴄʜ σwnєr");
  }
...
zokou({
  ...
  switch (option) {
    case "yes":
      ...
      responseMessage = 'Areact has been enabled♻️.';
      break;

    case "no":
      s.AUTO_REACT = 'no';  // Disable Areact
      responseMessage = 'Autoreaction has been disabled♻️.';
      break;
...
zokou({
  ...
  switch (option) {
    case "yes":
      ...
      responseMessage = ' has been enabled successfully♻️.';
      break;

    case "no":
      s.AUTO_READ_STATUS = 'no';  // Disable autoread status
      responseMessage = ' has been disabled successfully♻️.';
      break;
...
zokou({
  ...
  // Check if the command is issued by the owner
  if (!superUser) {
    return repondre("*This command is restricted to the bot owner or Sʜᴀᴅᴏᴡ-Xᴛᴇᴄʜ owner.* 💀,,idiot");
  }
